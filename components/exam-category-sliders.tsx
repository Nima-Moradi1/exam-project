...
type SliderExam = { href: string; label: string; description: string; duration: string; difficulty: string; accent: ExamCardTheme; category: string; categorySlug: string; level: string; levelSlug: string };
...
function Carousel({ path, exams }: { path: LearningPath; exams: SliderExam[] }) {
  ...
  return <section className={`exam-category-slider${canNavigate ? "" : " exam-category-slider--single"}`} onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocusCapture={() => setPaused(true)} onBlurCapture={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setPaused(false); }}>
    ...
    <div ref={viewportRef} className="exam-category-slider__viewport" role="region" aria-roledescription="carousel" aria-label={`آزمون‌های ${path.name}`}><div className="exam-category-slider__track">{exams.map((exam, index) => <article className={`hub-exam-card exam-card--${exam.accent}`} key={exam.href} ref={(element) => { itemRefs.current[index] = element; }} role="group" aria-roledescription="slide" aria-label={`${(index + 1).toLocaleString("fa-IR")} از ${exams.length.toLocaleString("fa-IR")}`}><div className="exam-card__attributes"><span className={`exam-topic-chip exam-topic-chip--${chipThemes[exam.categorySlug] ?? "mint"}`}>{labelFor(exam.categorySlug, exam.category)}</span><span className="exam-duration-badge">{exam.duration}</span></div><h4>{exam.label}</h4><p>{exam.description}</p><div><strong className="exam-difficulty">{exam.difficulty}</strong><NavigationLink className="hub-exam-card__action" href={exam.href}>مشاهده و شروع <ArrowIcon className="hub-exam-card__action-icon" /></NavigationLink></div></article>)}</div></div>
