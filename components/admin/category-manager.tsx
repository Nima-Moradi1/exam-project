...
export function CategoryManager({ tree }: { tree: CategoryTreeNode[] }) {
  ...
  return (
    <div className="admin-stack">
      ...
      <div className="admin-tree" role="tree" aria-label="درخت دسته‌بندی‌ها">{items.map((item) => <div className="admin-tree__row" key={item.id} role="treeitem" aria-level={item.depth + 1}><span style={{ paddingInlineStart: `${item.depth * 24}px` }}><strong>{item.name}</strong><small dir="ltr">/{item.slug}</small></span><span>{item.status}</span><div><button type="button" onClick={() => run(() => reorderCategory({ categoryId: item.id, direction: "up" }))} disabled={pending} aria-label={`انتقال ${item.name} به بالا`}>↑</button><button type="button" onClick={() => run(() => reorderCategory({ categoryId: item.id, direction: "down" }))} disabled={pending} aria-label={`انتقال ${item.name} به پایین`}>↓</button><button type="button" onClick={() => run(() => archiveCategory(item.id))} disabled={pending}>بایگانی</button></div></div>)}</div>
    </div>
  );
}
