...
export default async function CategoryPage({ params }: { params: Promise<{ segments: string[] }> }) {
  ...
  return (
    <main id="main-content" className="catalog-page page-shell" lang={category.locale} dir={direction}>
      ...
      {content.children.length > 0 && <section className="catalog-section" aria-labelledby="children-title"><div className="catalog-section__heading"><div><span className="eyebrow"><i /> گام بعدی</span><h2 id="children-title">دسته‌بندی‌های این بخش</h2></div><p>برای محدودتر کردن موضوع، یکی از زیرشاخه‌ها را انتخاب کنید.</p></div><div className="catalog-grid">{content.children.map((child) => <NavigationLink className="catalog-card" key={child.id} href={`/categories/${[...segments, child.slug].join("/")}`}><h3>{child.name}</h3>{child.description && <p>{child.description}</p>}<strong>مشاهدهٔ دسته‌بندی</strong></NavigationLink>)}</div></section>}
      <section className="catalog-section" aria-labelledby="exams-title"><div className="catalog-section__heading"><div><span className="eyebrow"><i /> آزمون‌های آماده</span><h2 id="exams-title">آزمون‌های منتشرشده</h2></div><p>سطح و زمان هر آزمون را بررسی و سپس شروع کنید.</p></div>{content.exams.length ? <div className="catalog-grid">{content.exams.map((exam) => <NavigationLink className="catalog-card" key={exam.id} href={`/exams/${exam.slug}`}><h3>{exam.title}</h3><p>{exam.shortDescription}</p><small>{Math.ceil(exam.durationSeconds / 60)} دقیقه · {exam.difficulty}</small><strong>مشاهدهٔ آزمون</strong></NavigationLink>)}</div> : <p className="empty-state">فعلاً آزمون منتشرشده‌ای در این دسته وجود ندارد.</p>}</section>
    </main>
  );
}
